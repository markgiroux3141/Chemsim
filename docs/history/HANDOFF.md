...
108. ✔✔ **C4 — THE ABE FERMENTATION: THE CLASS M5 REFUSED WAS AN OUTCOME LABEL,
    AND ITS LUMP WAS A FORMATTING ARTEFACT.** M5 refused `fermentation` as *"a
    metabolic NETWORK, not a transformation"* and `PLAYABLE.md` §8b priced it as
    the biggest single class left at **+2 playable**. Both were right and neither
    said what to build. Playable **18 → 20** (tiers 10 / 9 / 1), runnable
    **41 → 42**, classes **55/236 → 57/240**, template-ready **44 → 45**, BOTH
    **36 → 37**; species-ready unchanged at **85** — four SMARTS strings, a
    five-way taxonomy split, two table entries, no data row and no engine code.
    `validation/fermentation.py` (8 panels, ~30 s),
    `tests/test_fermentation.py` (31 tests, ~70 s).

    ⚠⚠⚠ **THE ARITHMETIC THAT DECIDED IT, DONE BEFORE ANYTHING WAS WRITTEN:**

        abe-fermentation 1, as written  C6H12O6 -> C10H24O5             NO
        ... and it balances only at     5 glucose -> 2 A + 2 B + 2 E
                                        + 12 CO2 + 8 H2
    ...
    **Clostridial solventogenesis is three branches off one pyruvate node, and
    the 5:2:2:2:12:8 vector was the arithmetic of writing them on ONE LINE.**
    Nothing consumes five sugars. ⚠ So `corpus_balance` passed the row for the
    same reason it passes `vanillin-lignin` — and the two need **opposite**
    answers: the lignin row is short a PRODUCT and must be left wrong, this one
    was short a LINE BREAK and could just be split. *A coefficient vector cannot
    tell those apart; only reading the chemistry can.*

    ⚠⚠⚠ **FOUR SESSIONS RUNNING.** C1: a price for a species not in its
    chemistry. C2: a price in a different table. C3: a class refused off one of
    its rows. C4: a class refused off its row's **formatting**.

    ⚠⚠⚠ **AND THE CLASS HAD TO BE SPLIT FIVE WAYS OR THE +2 WOULD HAVE BEEN A
    FALSE CREDIT.** Five rows, five mechanisms: `solventogenic-fermentation`
    (built), `homolactic-fermentation` (built),
    `aerobic-overflow-fermentation`, `amino-acid-fermentation`,
    `secondary-metabolite-fermentation` (named gaps). A template written off
    `abe-fermentation` cannot make citric acid, glutamic acid or penicillin G out
    of a sugar. **The headline cost is +4 on the DENOMINATOR against +2 covered**
    — S7's rule, *a split that lowers the headline is a split working* — and G4's
    *only RUNNING it said so* arrived **before** the run for once, because the
    rows were read first.

    ⚠⚠⚠ **THE ENGINE FINDING IS A REFUTATION OF §M10's OWN CHEAP VERSION, AND IT
    FAILS WORSE THAN THE TWO DOCSTRINGS THAT DESCRIBE IT.** M10 scopes the
    Michaelis-Menten plateau as *"a declared order of ZERO in the substrate ...
    needs no kernel change"*. Run at order zero past the end of the sugar:
    ...
    **The substrate is CLAMPED at zero in the reported state while the products
    ...
    simulated hours.** 1.79 mol of ethanol out of 0.5 mol of glucose.
    ⚠⚠ **`conservation_report()` sees every mole and calls four tenths of one
    "round-off it could not settle"** — the guard is load-bearing and its own
    label is calibrated for the case it was written for. *Same shape as
    "energy_terms lies unless given the run's own boundary state".* **M10 stays
    open; its cheap door is measured shut.**

    ⚠⚠ **WHAT IS FITTED IS SAID TO BE FITTED, AND ONE NUMBER IS NOT.** Three
    pre-exponentials were set to the classical 3:6:1 solvent slate by mass
    (2.38:3.73:1 by mole; the flask gives 2.40:3.75:1 at 48 h) and to a 48-hour
    batch (77.6% conversion). ⚠⚠ Evans-Polanyi was **refused** here: three
    branches 220 kJ/mol apart in dH would predict nothing but butanol, and a real
    slate is genetic regulation. **Selectivity between two CHEMICAL templates is
    derivable in this project (S11); between two METABOLIC branches it is not.**
    ⚠⚠⚠ **The FERMENTATION GAS is the check: CO2 61.94% / H2 38.06% against a
    reported ~60/40, with nothing aimed at it** — H2 comes only from the acetonic
    branch, so it falls out of the slate and the three stoichiometries. ⚠ And two
    invariants hold to solver precision at every point: **H2/acetone is exactly
    4.000000000000**, CO2 is `3A + 2B + E` to nine figures.

    ⚠⚠ **THE ORGANISM IS NOT A SPECIES, AND THAT IS THE SESSION'S HOLE.** Every
    other gate here is chargeable — an acid, a base, a lattice, a voltage. This
    one is ALIVE, the corpus has no graph for a Clostridium, and `_maybe_catalyse`
    needs one. **A flask of sterile sugar water ferments**, the hole is under all
    eight of M10's biological routes, and it is why `Ea` is an apparent barrier
    over twenty enzymatic steps. ⚠ Every yield is an UPPER BOUND for a new
    reason: a real batch stalls at ~20 g/L of butanol because **butanol dissolves
    the organism that makes it**, and nothing here can express a product
    poisoning a catalyst that is not in the flask.

    ⚠ A sealed fermenter reaches **24.7 bar** at 96 h on its own gas and nothing
    was told to; vented it sits at 1.01 bar with the **conversion unchanged to
    1%**, because no branch is reversible. *A hazard, not a ceiling* — unlike the
    vanillin digester.

    ⚠ **TWO SUBSTRATE REFUSALS, BOTH DELIBERATE.** Sucrose is inert to all four
    templates (the anomeric carbon must carry an -OH), so **a brewer has to
    invert the sugar first** and `ethanol-fermentation` step 1 is load-bearing.
    Fructose is inert too and that one is a corpus limit: the corpus spells it a
    **FURANOSE**. *S7's pyranose/furanose finding, costing a SUBSTRATE this time
    rather than an equilibrium constant.* Mannose IS eaten, correctly.

    ⚠ Every branch prints M5's **MIXES STANDARD STATES** notice — glucose's
    vapour pressure at 298 K is below the standard-state floor while its products
    all shift, so the conventions differ by 64–219 kJ/mol in dH and the sign of
    dS flips. **It costs the K and nothing else**: dG is −121 to −353 kJ/mol on
    either basis. **Do not quote a K for a fermentation in this project.**

    ⚠⚠⚠ **AND A STEREOCENTRE TURNED UP A KEYING BUG NOTHING WAS LOOKING FOR.**
    ...
    L-lactic acid and one D-**, two species where the corpus has one. Spelling
    the four ring centres `[C;H1;@,@@:n]` removes it (RDKit's own rule), which is
    C3's isoeugenol decision at a stereocentre. Measuring it:

        corpus rows whose SMILES carries a stereo marker     146
        ... which PRICE OFF A DIFFERENT SOURCE when flat      31
              the PHYSICAL half moved                         30
              the FORMATION half moved                         2

    **The two halves of a ThermoData are keyed the OPPOSITE way round.** The
    physical tables carry the chiral spelling — sorbitol reaches a measured Tb
    chiral (YAWS 704.0 K) and falls to Joback flat (888.2 K), **184 K away**, and
    29 rows are that shape. The formation table carries the FLAT spelling —
    lactic acid flat reaches an experimental record and the corpus's chiral
    spelling falls to Benson. ⚠⚠⚠ **For 31 compounds the data tier is selected by
    an orthographic accident, and a spelling carries no thermochemical
    information at all** (no estimator here tells one enantiomer from another).
    ⚠ **NOT FIXED**: the fix is a stereo-insensitive **FALLBACK** in the lookup
    (S6's rule — a fallback, never an override) and it touches the provider every
    number in this project comes out of. Recorded with a size.

    ⚠⚠⚠ **THE CEILING MOVED FOR THE FIRST TIME SINCE C1: 41 → 45, AND THE WORK
    ORDER GREW 20 → 23.** A fermentation puts acetone, ethanol, butanol and —
    through `acetic-fermentation` — acetic acid on the shelf, feeding four routes
    that were not fed before (`acetic-anhydride-ketene`, `chloral-route`,
    `mercury-fulminate-route`, `white-lead-route`). **The goal a session is
    measured against is not a constant**, and two sessions of it sitting still
    were a property of what they built. ⚠⚠ **§8b has no +2 row left**: six
    classes tied at +1, 23 at +0, ten of which no template can buy at any price.
    ⚠ `ethylene` was re-priced from **+2 to +1** by a session that never touched
    it; `aluminium` is the sole +2 now.

    ⚠⚠ **THE SECOND ROUTE IS BOUGHT BY A BRANCH THAT IS NOT THE TARGET.**
    `abe-fermentation`'s catalog target is propanone; what unblocks
    `acetic-fermentation` is the **ethanol**, the minority branch at a seventh of
    the butanol. So the target-only shortfall in `test_playable.py` **moved
    4 → 5, the first move in five sessions**, by the same mechanism as the zinc
    retort's carbon monoxide. *A rule kept on a zero difference (the fouling row)
    and a rule kept on a growing one (the byproduct shelf) are different bets,
    and both are printed.*

    ⚠ **C3's DISCIPLINE WAS KEPT.** Every artefact was regenerated and the tests
    that pin it were run in the same breath: six `test_playable` assertions and
    two `test_vanillin` ones moved, and one `test_vanillin` test was **RENAMED**
    — `test_the_work_order_shrank_and_the_ceiling_did_not_move` asserted a claim
    about C3's own numbers that C4 falsified, so it is now
    `test_vanillin_feeds_nothing_and_that_is_still_true`, which is the claim
    worth pinning. ⚠⚠ **`test_the_PAIR_is_worth_more_than_the_sum_of_its_parts`
    survived with four numbers changed and its finding untouched, because it
    asserts DIFFERENCES.** *A test that pins a claim about a difference must
    assert the difference.*

    ⚠ `tolerance_audit.py` is **NOT owed**: no RHS edit and no data-table edit.
    Its last measured state is still C2's.

    ⚠⚠ **THE SUITE: 1159 passed / 0 failed in 26:09, run alone.** Per test,
    G6 / C2 / C3 / C4 are **1.3234 / 1.6363 / 1.3250 / 1.3542 s** — three
    sessions inside 2.4% and C2 24% above all of them, so **C3's re-pricing of
    C2's "+30% that nothing explains" holds.** ⚠ Individual big rows still
    move 4-10% with no cause (catalysis +10.2% here while the rig test went DOWN
...
115. ✔ **P3 + P4 — THE SHELF IS DATA, THE PICKER IS BUILT, AND PLAYING IT
    FOUND SIX TEMPLATE FIELDS THAT NEVER REACHED THE ENGINE.**
    `data/catalog/shelf.psv` (71 rows, three tiers), `tools/build_shelf.py`,
    `src/chemsim/engine/shelf_data.py` (generated), `engine/inventory.py`, a
    Bench tab, a REACT FURTHER control, `validation/shelf.py`, and
    `SAVE_VERSION` **8**. The loop is playable: open a picker, pour two things
    into a flask, do something, read what came out, put it back on the shelf.

    **THE SHELF.** 43 `natural` / 24 `intermediate` / 4 `bottle`, plus a cheat
    axis of all 1167 priced species and a picker roster of all 1583 with the 416
    refusals greyed and carrying the engine's own reason. ⚠ 45 natural species,
    43 rows: `coal-marker` and `collagen-marker` have no molecular graph, so
    neither can be a `VesselState` (§8.6 forbids it), and the marker set is
    pinned as an EQUALITY so a third cannot appear silently. ⚠ Seven natural
    rows are REFUSED a price and stay anyway — gold, quartz, pyrite,
    pyrrhotite, pyrolusite, borax, cryolite — because *you can dig this up* is
    true whatever the estimators say, and the day one is curated the row lights
    up with no edit to the file. `tests/test_playable_levers.py` asserts the
    three tiers against panel 3 of the audit they came from, so a route becoming
    reachable produces a failure message that IS the work order.
    ...
    correctly, generates a clean report, and **puts five shelf rows into the
    flask as matter no mechanic in this engine can touch.** Measured, 0.5 mol
    into 30 mol of water at 298 K for 600 s:
    ...
    A solid is held two ways with disjoint mechanics — the LATTICE calcines,
    roasts and reduces (`solid_state`, `surface`); its IONS in the solid block
    dissolve and precipitate through a Ksp (`PrecipitationArrays`: *"the lattice
    is not a species and never becomes one"*) — and **nothing converts one into
    the other**, which `examples/lime_cycle.py` states in a comment. Rock salt,
    fluorite, saltpetre, phosphate rock and anhydrite have NO solid-state or
    surface reaction, so dissolving is all they do; rock salt is the chlor-alkali
    feedstock and `validation/phosphate_rock.py` had already recorded that
    *without the lattice the rock is INERT*. **C2 measured the failure mode this
    rule would have walked into, a session before it was written.** So the rule
    is mechanism-driven, read off the engine's own declarations, and ⚠ rules 1
    and 2 COLLIDE on six rows where rule 1 wins and costs them their dissolution
    (calcite, covellite, galena, sphalerite, cinnabar, green vitriol —
    limestone in acid does nothing). **A NAMED ENGINE GAP, not a preference.**
    ⚠ And the coverage audit's own tier answers a different question: seven rows
    audit as `ion` and are rocks, because the audit asks *can this be priced* and
    the shelf asks *what species is in the bottle*.

    ⚠ **THE PHASE COLUMN IS A DECLARATION AND OLIVE OIL IS WHY.** Joback gives
    triolein `Tm = 828.9 K`, so a derived phase puts a bottle of olive oil in the
    SOLID block — 550 K wrong, an estimator outside its domain one rung further
    out than the element floor. One row in 71 disagrees with the engine and the
    generator prints it. ⚠ A Henry's-law species is a GAS: reading
    `Volatility.coefficient()` as a pressure put nitrogen, oxygen and carbon
    dioxide in the liquid on the first pass.

    **THE PICKER, AND CHOOSING ROWS BUILDS THE WORLD.** P2's handoff:
    `Vessel.charge_state` refuses a species the network does not carry and a
    network is derived from its FEED, so `inventory.scenario_for` owns the two
    guarantees a widget cannot be trusted with — every charged species in
    `feed_species`, and `electrolyte` on whenever an ion is charged.
    `examples.bench` is an ordinary `Example`, so Reset/Save/Load need no special
    case. **REACT FURTHER raises the bound**, and ⚠⚠ **it raises the SPECIES CAP
    too**: the bounds compete, four bench reagents hit 400 species at
    `generations=2`, and glucose/water/air give 400 species and 653 reactions at
    *both* 2 and 3 generations — so a button that only bumped `generations` would
    rebuild an identical network and look broken. **P1 found the same competition
    from the other side.** ⚠ The save format had to be fixed first: it held
    `{"example": key, "script": [...]}`, and a bench world has no key while a
    reacted-further world differs from its key's scenario by exactly the bound
    that was raised. It carries `Scenario.to_dict()` now; a pre-P4 file still
    opens.

    ⚠⚠⚠ **THEN IT WAS PLAYED, AND THE PLAY FOUND TWO THINGS NO TEST COULD.**
    Sulfur, air, water and a trace of NO2 off the shelf — the game's own chain 2
    — would not make vitriol.

    **(a) The bench's library was false by more than half.** It collected only
    `*_chemistry` bundles, which is the rule `validation/playable_levers.py`
    panel 5 uses, and silently skipped every template exported as a function of
    its own: `sulfur_combustion`, `sulfur_trioxide_hydration`, `lead_chamber`,
    `esterification`, `cannizzaro` and about forty more. The flask gave **four
    species, no reactions and an EMPTY FRONTIER at every generation count** — the
    engine correctly reporting a library with no sulfur chemistry in it — while a
    blurb claiming *every template in the project* was on screen. The sweep is by
    RESULT TYPE now (50 templates), so a naming convention nobody promised to
    keep cannot fool it.

    **(b) ⚠⚠⚠ `TemplateSpec` WAS DROPPING SIX `ReactionTemplate` FIELDS, AND A
    FRONTEND CAN ONLY REACH THE ENGINE THROUGH A `Scenario`.**
    `sulfur_combustion` declares `orders=(1, 1, 0...)` — first order in oxygen,
    which S11 spent a session establishing — and the network ran the SMARTS' own
    **ninth-body mass action** instead. 0.02 mol S8 in a sealed litre at 700 K
    for an hour:
    ...
    **A threshold where the declared law is a straight line**, and the shelf's
    own oxygen bottle sits at 0.05. The same drop **un-gated every heterogeneous
    catalyst** (`solid_catalyst`, S1 — eleven templates declare one, and without
    it ammonia synthesis runs in a flask with no iron), **took the driving force
    out of every electrode reaction** (`electrons`, M8), and **lost G2's ring
    deactivation** (`hammett_rho`, and `aromatic_nitration()` ships with **-6.5**,
    so this was the DEFAULT being lost — every stage of a staged nitration ran at
    the same rate, and `examples/dropping_funnel.py` and
    `validation/dropwise.py` both build that template through a spec).
    `SAVE_VERSION` → **8**: not a verb and not a structure, **the same bytes mean
    something different**, which is the strongest reason to bump there is.

    ⚠ **THREE OF THE SIX WERE FOUND BY THE PLAY AND THREE BY A TEST, AND THE
    TEST ONLY FOUND THEM BECAUSE IT ASSERTED THE *SET* OF FIELDS.** The play
    reached `orders`; writing `tmpl_fields <= spec_fields` turned up the three
    `hammett_*` ones immediately. *`alpha`'s own comment already said a template
    field is not finished until it round-trips — the lesson is that the assertion
    has to be about the set rather than about whichever field somebody
    remembered.* ⚠ And restoring `solid_catalyst` puts six catalyst lattices
    (nickel, iron, copper, cobalt, tenorite, hematite) into every bench network
    as species at zero moles, which is the zero-Jacobian-column case S5 bounded.

    **AND THEN IT WORKED:** SO2 at one generation, **SULFURIC ACID at two**, and
    an empty frontier at three. Chain 2 out of the picker in two presses of a
    button, from four natural rows and one intermediate. ⚠ The NO2 is why the
    `intermediate` tier exists — there is no template for SO2 + O2 → SO3 without
    a carrier, so sulfur, air and water alone stop at SO2 **with an empty
    frontier**, the engine saying it knows no further chemistry rather than
    declining to look. ⚠⚠ **AND THE SHELF'S OWN WATER BOTTLE STOPS THE
    SHELF'S OWN SULFUR BURNING** — the play's last finding, and it is emergent
    rather than declared. A gas-phase combustion is first order in GASEOUS S8,
    and 5 mol of water (90 mL) holds the sulfur in the liquid. Sealed, 700 K, one
    hour, 0.05 mol of oxygen throughout:
    ...
    **A tenth of the water is 7.7x the sulfur in the vapour and four orders of
    magnitude of conversion.** *You do not burn sulfur in a wet flask*, and the
    phase model says so without being told. ⚠ An earlier draft of this item said
    "seal it and the same charge burns" — that is FALSE, extrapolated from the
    0.02 mol row: sealing changes almost nothing at 5 mol of water, and the lever
    is how much water you pour. ⚠ The bench flask also VENTS, so an open flask at
    700 K passes its steam and oxygen out of the top and makes 3e-5 mol of SO2 in
    an hour; and the shelf's gas bottles are deliberately small, so 0.2 mol of S8
    against 0.05 mol of O2 is 32x short of stoichiometry by construction.

    New: `tests/test_shelf.py` (18), 7 in `tests/test_ui.py`, 3 in
    `tests/test_protocol.py`, 1 in `tests/test_playable_levers.py`.
    `validation/shelf.py` is the standing audit. Scoreboard unchanged: 21 of 173
    ...
    than to the constant, so it survived the grep. Fixed by comparing to
    `SAVE_VERSION` — *a test that pins a version to a literal has to be edited in
    two places by every session that moves it, and the second is the one that
    gets forgotten.* The suite was NOT re-run after the fix, stated rather than
    skipped: the edit is one assertion inside the test that failed plus its
    docstring, and that file passes 22/22 in 15 s. **So the figure is 1255/1 with
    ...
    *Left standing above because it is what P4 measured and the reasoning for
    ...
    move, because **no audited example builds a real template through a spec** —
...
116. ✔ **R-SERIES OPENED — "REACT UNTIL DONE" WAS MEASURED RATHER THAN
    ...
    *bounding SIZE does not shrink the search, it REDIRECTS it into a denser
    ...
    CRASHES THE BUILD IN TWO CLICKS.** (✔ Closed by R1, item 117 — and it got
    worse a THIRD time on being fixed: it is five species, not one.) It was
    handed over as *"deeper exploration crashes rather than degrades"*.
    Measured: picker rows **`5-HMF` + `oxygen`, both offered UNGREYED, at
    `generations=1`** → a bare
...
117. ✔ **R1 — AN UNPRICEABLE SPECIES IS THE FOURTH REPORTED COVERAGE LIMIT,
    AND CLOSING THE CRASH CHANGED THE ANSWER RATHER THAN THE FAILURE MODE.**
    2026-09-01. `network/builder.py` `_unpriceable` screens every NEW product
    BEFORE `_concrete_reactions` runs — pricing is what that call does — records
    it against its own refusal in `_ExpansionState.unpriced`, drops **the whole
    rewrite** the way the `too_big` branch does, and reports it on the same
    `reports()` channel that carries `max_species` and `max_molar_mass`. The
    structured companion is **`ReactionNetwork.unpriced`**, `{smiles: refusal}`,
    on the same footing as `unexpanded`; the notice quotes the refusal
    **verbatim** for the first three, because the refusal already names the
    species, says which half of its record resolved and says what would fix it.
    *The work was routing it, exactly as `MILESTONES.md` predicted.*

    ⚠⚠⚠ **THE CRASH WAS HIDING THE REAL ANSWER, AND THAT IS THE FINDING.** A
    traceback reports the FIRST refusal and stops. With all of them reported,
    the picker's own two-row pick is **five species from three templates** —
    `aerobic_oxidation` → 2,5-diformylfuran, `ether_condensation` → its ether
    dimer, `friedel_crafts_hydroxyalkylation` → three bis-furylmethanes — and
    with all five refused **the flask has 8 species and ZERO reactions**. The
    engine cannot price *any* chemistry it can find between 5-HMF and oxygen.
    That is a statement about the pick that no exception could make. ⚠ It also
    re-prices this same finding for the third time: *"deeper exploration
    crashes"* → *"one generation off the picker's own roster"* → *five species,
    zero reactions.*

    ⚠⚠ **THE DESIGN QUESTION WAS NOT THE ONE THE WORK ORDER PREDICTED.** The
    predicted one — dropping a species touches matter, so argue it the way §8.2
    argues the generation bound — went in as written. The one that bit is that
    **ONE PROVIDER RAISES TWO REFUSALS AND ONLY ONE IS A COVERAGE LIMIT**:
    *no thermochemistry available* means NO SOURCE ANYWHERE prices it (a DATA
    gap, and it drops), while the element floor's refusal means **THIS provider
    is the wrong one** and names the right one (a SETUP gap, and it must not).
    Treating them alike **broke two green tests** — `saponification`'s stearate
    ion under a neutral provider, and the kolbe dianion `test_furans.py` pins as
    a raise — which is how the distinction was found rather than argued. The fix
    is a **type**, `properties/thermochemistry.OutsideEstimatorDomain`, a
    `ValueError` subclass so every existing `except ValueError` is unchanged;
    both tests went green again **with no test edits**, which is the evidence
    that the distinction belongs to the code and not to the tests.

    ⚠ **AND THE SETUP GAP IS SAFE TO PASS THROUGH FOR A MEASURED REASON.**
    ...
    *before* consulting thermochemistry, which is why an ionic product under a
    ...
    price it is a reversible template, and `_concrete_in_phase` already catches
    that and re-raises naming the reaction, the phase and the provider to use.
    **A loud refusal that names its own fix is the right answer to a
    misconfigured network; a missing MEASUREMENT is the one nobody can act on,
    and that is the only one that drops.**

    It costs nothing: `ThermochemistryProvider.get` caches on success, so a
    surviving species is priced once and read from the cache two lines later,
    and `state.unpriced` is the failure cache the provider does not keep.
    `validation/shelf.py` panel 5E is rewritten from *"if this line prints, R1
    is done"* into asserting the notice; four tests are pinned in
    `tests/test_robustness.py`.

    ⚠⚠ **THE SUITE WAS RUN ON R1's OWN CODE AND IT IS GREEN: 1260 PASSED /
    0 FAILED IN 30:01** (1801.36 s, **1.4297 s per test**, second-lowest of
    eight, spread unchanged at 11.0%). That run is the evidence R1 needed
    specifically: R1 changes what a network CONTAINS, so no targeted run could
    have settled the blast radius. It is zero — nothing moved but R1's own four
    tests. ⚠ And the two regressions R1 *did* cause were caught by a four-file
    targeted run **before** the suite, which is the argument for running the
    cheap set first rather than only at the end.

    ⚠⚠⚠ **`tolerance_audit.py` WAS RUN, THE PREDICTION FAILED ON ONE ROW, AND
    THE ROW WAS THE INSTRUMENT.** Owed from P4 and from R1 — both are network
    CONSTRUCTION. Eleven of twelve rows held byte-for-byte. `workshop` came back
    **1.95e-04** against a record of **1.98e-04** that had been stable across P1
    and C7. Chased rather than waved off, and all three obvious causes were
    refuted before the real one was found:

    * **not R1** — `workshop`'s four networks all report `unpriced` empty, and
      HEAD (pre-R1, in a throwaway worktree) already reads 1.95e-04;
    * **not BLAS threading** — capped twice and uncapped once, all 1.95e-04, so
      R2's thread capping is numerically neutral here;
    * **not run-to-run noise** — the value is deterministic in every repeat.

    **Bisected to `05609c4` (P3+P4).** `bc6999f` and `d352c44` both read
    1.98e-04.

    ⚠⚠ **AND THE MOVED LINE IS A JSON SAVE-FILE SIZE IN BYTES:**

        save = 10113 bytes of JSON      P2 and before
        save = 10237 bytes of JSON      P4 and after

    **The loose/tight gap is 2 bytes in both and never changed** — the saved
    JSON holds a float whose decimal form is two characters longer at rtol 1e-8.
    What moved is the DENOMINATOR: P4's six new `TemplateSpec` fields grew every
    save file by 124 bytes, so 2/10113 became 2/10237. `workshop`'s
    default-tolerance stdout is **byte-identical** across those two commits,
    which is the proof that nothing physical moved. *That row was never evidence
    about convergence, and every session that quoted 1.98e-04 was quoting the
    size of a JSON blob.*

    **FIXED IN THE INSTRUMENT, on this file's own precedent.** It already
    excises a wall clock as a TOKEN rather than a line, because the first version
    of the audit manufactured a `wait_until` finding out of *"0.07 s of wall"*.
    A serialized SIZE is the same class of number: something a run prints that
    says nothing about whether it converged. `scrub` now takes both, and the
    module **asserts its own behaviour on import** (`_MUST_SCRUB` /
    `_MUST_NOT_SCRUB`) — because both findings this file has ever manufactured
    came out of `scrub` and not out of a solver. ⚠ `1.25 bar` survives the size
    pattern only because of its trailing ``; that is load-bearing and is now
    asserted rather than assumed.

    **THE NEW STANDING RECORD** (R1, thread-capped, still exits 1 on the same
    two `serious` rows):
        ...
        mercury_retort      0 lines — and `mercury_retort` at 1.00x is the
                            harness's own self-check, so the rebinding works

    ⚠ **THE LESSON IS THE ONE THE AUDIT ALREADY TAUGHT ONCE.** *An instrument
    that cannot tell a wall clock from a result will manufacture findings* — and
    it took three sessions of a stable wrong number, plus a full bisect, to
    notice it had been doing it again with a byte count. The reason it was
    caught at all is that the number was RECORDED, so a 1.5% move in it was
    visible. **Recording a number you do not yet understand is what makes it
    checkable later.**

    ⚠⚠⚠ **AND R1 UNLOCKED PANEL 5F, WHICH REFUTED A CORRECTION THE R-SERIES
    HAD MADE THE SAME DAY.** Two of the sweep's four rows had always crashed on
    an unpriceable species — *"it turns two picks into crashes"* was exactly
    that — so `--mass-cap` was re-run:
    ...
    **The two new rows cost the finding its law.** The reaction count is NOT
    monotonic in the cap: 519 and 458 are BELOW the uncapped 644, so *a tighter
    bound makes a bigger network* is true at 250 g/mol and is not a rule — and
    it was stated as one only because the rows that contradict it were the rows
    that crashed. What survives is stronger and needs no arithmetic: **the cost
    is in the SEARCH and not in the RESULT**, since 500 g/mol spends 100 s to
    make FEWER reactions than 10.6 s of uncapped work.

    ⚠⚠ **AND THE 19x WAS WRONG AND THE ORIGINAL 35x WAS RIGHT.** The R-series
    overturned 35x to 19x on the grounds that the recorded 10.9 s uncapped build
    "does not reproduce", citing sub-panel B's live 19.8 / 20.1 / 20.2 s. **It
    reproduces — 10.5 and 10.6 s on re-running F itself.** B and F were never
    measuring the same thing: **B builds a `World`, F calls `build_network`.**
    Interleaved in one process on identical networks (400 species / 644
    reactions): `build_network` **11.14 / 10.67 / 10.85 s** against
    `World(bench)` **20.46 / 20.35 s** — not a warm cache, and it does not drift
    with order; the ~9.6 s gap is World and Vessel construction. So the R-series
    divided F's numerator by B's denominator, **the exact mistake it was
    accusing the first write-up of**, and like for like it is **36x**. ⚠ The
    consequence to carry: *"build" in panels 5B and 5C is a WORLD build and
    roughly half of it is not discovery* — which leaves panel C's conclusion
    untouched, because 10.8 s of discovery against 107 s of stepping is the same
    argument as 19.8 against 107. **A number quoted across two measurement paths
    is wrong however carefully it is divided**, and that is now the second time
    this one panel has taught it.
