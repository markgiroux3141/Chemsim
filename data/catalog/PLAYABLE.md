...
|---|---:|---:|---:|
| needs = `route_roles` *(wrong)* | 17 / depth 2 | 23 / depth 3 | 24 / depth 3 |
| needs = **order** *(correct)* | 16 / depth 2 | 22 / depth 3 | 23 / depth 3 |

⚠⚠⚠ **THE BOTTOM-RIGHT CELL IS THE ANSWER (23), AND THE TWO CELLS BESIDE IT ARE EQUAL — WHICH IS THE FINDING.** Under the correct needs rule, shelving byproducts-only costs *nothing*, so the fouling-row bug in rule 3 is **invisible**. It is only visible along the top row, where it is worth one route. **Two of the four rules were wrong at once and fixing the first one masked the second.** Had the needs rule been fixed first, rule 3 would have looked like a distinction without a difference and gone in wrong — and it would have started costing routes silently the moment the lead chamber became reachable.
...
| `lime-cycle` | calcium oxide | calcium-carbonate, water | **calcium-oxide**, calcium-carbonate, calcium-hydroxide, carbon-dioxide, water |
| `mercury-from-cinnabar` | mercury | mercury-sulfide, oxygen | **mercury**, sulfur-dioxide |
| `salicin-hydrolysis` | 2-hydroxybenzoic acid | oxygen, salicin, water | **salicylic-acid**, glucose, hydrogen-peroxide, salicyl-alcohol, salicylaldehyde, water |
| `starch-hydrolysis` | D-glucose | starch-unit, water | **glucose**, fructose, maltose |
| `tanning-route` | 3,4,5-trihydroxybenzoic acid | collagen-marker, tannic-acid-core, water | **gallic-acid**, glucose, tanned-leather-marker, water |
| `vitriol-distillation` | sulfuric acid | iron-ii-sulfate, water | **sulfuric-acid**, iron-iii-oxide, sulfur-dioxide, sulfur-trioxide |
| `zinc-smelting` | zinc metal | carbon-graphite, oxygen, zinc-sulfide | **zinc**, carbon-monoxide, sulfur-dioxide, zinc-oxide |

### Tier 2 (12)

| route | target | needs | puts on the shelf |
|---|---|---|---|
| `acetic-fermentation` | ethanoic acid | ethanol, oxygen | **acetic-acid**, acetaldehyde, hydrogen-peroxide, water |
| `bleaching-powder` | calcium hypochlorite | calcium-hydroxide, chlorine | **calcium-hypochlorite**, calcium-chloride, water |
| `copper-smelting` | copper metal | carbon-monoxide, copper-sulfide, oxygen | **copper**, carbon-dioxide, copper-ii-oxide, sulfur-dioxide |
| `hmf-route` | 5-HMF | fructose, sulfuric-acid | **5-hydroxymethylfurfural**, formic-acid, levulinic-acid, sulfuric-acid, water |
| `hypochlorite-bleach` | sodium hypochlorite | chlorine, sodium-hydroxide | **sodium-hypochlorite**, sodium-chloride, water |
| `lead-smelting` | lead metal | carbon-monoxide, lead-sulfide, oxygen | **lead**, carbon-dioxide, lead-ii-oxide, sulfur-dioxide |
...
| `vanillin-eugenol` | 4-hydroxy-3-methoxybenzaldehyde | eugenol, oxygen, sodium-hydroxide | **vanillin**, acetaldehyde, isoeugenol, sodium-hydroxide |
| `vanillin-lignin` | 4-hydroxy-3-methoxybenzaldehyde | lignin-monomer-coniferyl, oxygen, sodium-hydroxide | **vanillin**, sodium-hydroxide, water |
| `water-gas-shift` | hydrogen | carbon-monoxide, iron-iii-oxide, water | **hydrogen**, carbon-dioxide, iron-iii-oxide |

### Tier 3 (1)

| route | target | needs | puts on the shelf |
|---|---|---|---|
| `methanol-synthesis` | methanol | carbon-dioxide, carbon-monoxide, copper, hydrogen | **methanol**, copper, water |

After every tier has run, the shelf holds **87 species** (the 45 natural ones plus 42 made).
...
Grant a player one species free and re-run the fixed point. Base is 23.
...
| **+1** | `guncotton` | nitrocellulose repeat unit | — *none* | nitrocellulose-unit |
| **+1** | `pyrite-roasting` | sulfur dioxide | — *none* | iron-disulfide |
| **+1** | `acetic-anhydride-ketene` | acetic anhydride | `pyrolysis-dehydration` | — |
| **+1** | `chloroform-route` | trichloromethane | `haloform` | — |
...
| **+1** | `citric-acid-fermentation` | citric acid | `aerobic-overflow-fermentation`, `precipitation` | calcium-citrate |
...
⚠⚠ **2 OF THEM NEED NO TEMPLATE AT ALL** — every class they use is already covered and they are blocked purely on a species the engine refuses to price: `guncotton`, `pyrite-roasting`. **That is a data job, not a chemistry job**, and `pyrite-roasting` in particular is blocked on exactly the entry the engine queue already records as source-blocked — pyrite has an enthalpy in WEBBOOK and an entropy in nothing. **A data refusal is now measurably a playability blocker and not just a coverage one.**
...
⚠⚠ **AND 9 OF THE ROWS ABOVE CANNOT BE BOUGHT BY TEMPLATES AT ALL.** Grant every class each one is missing and it is still not runnable, because a refused species is blocking it too: `bayer-process`, `blast-furnace`, `calcium-carbide`, `coal-gas`, `frank-caro`, `gunpowder`, `hall-heroult`, `mercury-fulminate-route`, `white-phosphorus`. **They are joint grants priced as though they were single ones.**
