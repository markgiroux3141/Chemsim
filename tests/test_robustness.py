...
def test_an_unpriceable_ion_CHARGED_into_the_flask_is_refused_at_the_door():
    """T13: the caller's half of the same rule, enforced where the caller is.

    This test used to charge the saligenolate and pin that the flask built with
    one reaction dropped -- an ion "nothing asks about" riding along. T13
    measured what asks: ``vessel.build_phase_arrays`` prices EVERY species for
    its heat capacity and molar volume before a reaction is looked at, and
    refuses this one there. So that flask was built, reported and un-runnable,
    and ``to_arrays`` raised one call after the notices.

    Dropping a charged species is not available -- it would delete matter the
    player put in the flask -- so the caller's half is a refusal, and it belongs
    at the call that charged it rather than at the call that integrates it.
    What the TEMPLATES make is still droppable and still reported; the test
    below is that half.
    """
    ...
    with pytest.raises(ValueError, match="was CHARGED into this flask") as err:
        build_network(["OCc1ccccc1O", "O", SALIGENOLATE], list(full_library()),
                      thermo=thermo, volatility=VolatilityProvider(thermo),
                      max_species=60, generations=1)
    assert SALIGENOLATE in str(err.value), "the refusal names the species"
    assert "no AcidPair" in str(err.value), "and what would fix it"

    # The same flask without it builds and INTEGRATES: saligenin dissociating
    # still reaches the same ion, and that half is a notice, not a refusal.
    net = build_network(["OCc1ccccc1O", "O"], list(full_library()),
                        thermo=thermo, volatility=VolatilityProvider(thermo),
                        max_species=60, generations=1)
    assert SALIGENOLATE in net.unpriced
    assert SALIGENOLATE not in net.species
...
def test_an_unpriceable_ion_a_template_MAKES_drops_its_rewrite_either_way():
    """T13: the screen no longer asks whether the TEMPLATE needs the price.

    T1d kept an ion whose maker does not price it -- ``saponification`` is
    irreversible with alpha = 0, so it derives no barrier and no reverse rate --
    on the reasoning that an ion nothing asks about sits in a network perfectly
    well. The vessel asks. ``build_phase_arrays`` needs a heat capacity and a
    molar volume for every species, so the reactions that branch preserved were
    reactions in a flask no integrator could accept.

    Measured on the flask below, a triglyceride of a BRANCHED acid so the
    carboxylate falls outside T18's plateau domain (the stearate T1d used is
    priced by the rule now, which is why its own witness had gone): 10 species,
    7 reactions and ``to_arrays`` raising, against 4 species, 0 reactions and a
    network that integrates. The seven reactions are the cost and they were
    never runnable.
    """
    from chemsim.properties import VolatilityProvider

    thermo = electrolyte_provider()
    assert not saponification().uses_thermochemistry, "nothing here needs it"
    tri = "CCC(C)C(=O)OCC(OC(=O)C(C)CC)COC(=O)C(C)CC"
    net = build_network([tri, "[OH-]", NA, WATER], [saponification()],
                        thermo=thermo, volatility=VolatilityProvider(thermo),
                        max_species=60)
    assert list(net.unpriced) == ["CCC(C)C(=O)[O-]"]
    assert not net.reactions, "the rewrite goes with the species"
    assert [n for n in net.notices if "could not be PRICED" in n]
    net.to_arrays()                       # the whole point: it is runnable now
