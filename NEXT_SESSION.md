...
## C6 — THE RIG SINGULARITY, AND IT WAS A PUMP RUNNING DRY

⚠⚠ **WHAT C6 RAN AND WHAT IT OWES.** `validation/dropwise.py` (new panel 1b),
`examples/dropping_funnel.py`, `tests/test_rig.py`, `tests/test_dropping_funnel.py`,
the full suite, and **`validation/tolerance_audit.py`, which WAS owed** — C6
edited the RHS (the METER branch is inside `RigIntegrator.make_rhs`), so C5's
*"no RHS edit and no data-table edit"* exemption did not carry over. ⚠ **No
catalog artefact was regenerated and none needed to be**: C6 bought no route, no
class and no species, so `COVERAGE_REPORT.md`, `PLAYABLE.md` and `ROUTE_INDEX.md`
are byte-identical to C5's.

**Scoreboard UNCHANGED and deliberately so:** 21 playable (tiers 10 / 10 / 1),
44 runnable, 59/240 classes, 46 template-ready, 38 BOTH, species-ready 85,
ceiling 45. **One engine line, two tests, one validation panel.**

| what | the value | where it is pinned |
|---|---|---|
| ⚠⚠⚠ **the fragility was filed in the WRONG LAYER, on a TRUE measurement** | C5 filed it as *"a numerics session on the rig integrator"* with evidence taken both ways round — and the evidence was sound while the conclusion was inverted. **A permutation changes which `h` `num_jac` lands on; the number it was scaling was meaningless at every `h`** | MILESTONES §C6 §1. *Six sessions running the named blocker was not the blocker* |
| ⚠⚠⚠ **it is the SPARSE LU path that raises** | `useful_sparsity` grants a pattern (**62 groups of 82 columns**), so `num_jac` returns a sparse `J` and BDF calls `splu`. **"Factor is exactly singular" is SuperLU's message.** The identical network on the DENSE path runs and lands on the same answer | measured at caps 10/14/15, both paths. **A rank-deficient `I - c*J` is a hard crash on one path and a rejected step on the other** |
| ⚠⚠⚠ **and the matrix is NOT singular — it is SCALED** | no zero rows, no zero columns, no duplicate rows or columns; `lu_factor` accepts it with **min\|U_ii\| = 1.5064e-03, zero pivots, no warnings**. **cond = 4.038e+23** | ⚠ LAPACK's default `matrix_rank` says 26 of 122 and that is an artefact of the dynamic range, not a rank |
| ⚠⚠⚠ **the 1e+19 entries are not derivatives** | `f` is **CONSTANT at −1.322448 across twenty decades of `h`** (1e-20 to 356). It is a STEP, so the quotient is exactly `Δf / h` — **1.6e+20 at h=1e-20, 1.6e+09 at h=1e-09.** `num_jac` reports its own probe size | the h-sweep. ⚠ Same shape as `jacobian.py`'s burner column, from the opposite side: there the derivative was projected away, here it is a discontinuity |
| ⚠⚠⚠ **the step is a COMPOSITION TAKEN OVER NOTHING** | the funnel holds **7.30e-26 mol**; adding **1e-20 mol** takes one species from 0% to **99.9993%** of the layer. A meter carries the donor's composition AND its enthalpy, so `f[pot.T]` steps **+0.2903 → −1.3224** | **the control is exact: the same probe on the POT, holding 1.10 mol, moves it by 0.000000e+00.** *A mole fraction is scale-invariant, so an empty vessel's composition is infinitely sensitive* |
| ⚠⚠⚠ **the guard was a 0/0 clamp doing a GATE's job, already forbidden in writing** | `... if tot_a > 0.0 else zeros`, against `MOLE_FRACTION_DENOM`'s own comment: *"a clamp that exists to avoid 0/0 must not double as a second gate"* | **the exact defect, one module over, stated long before it was met here** |
| ⚠⚠ **a METER is the ONLY edge exposed, and structurally** | a VAPOUR edge is `k dP x_a` with `dP` ∝ the same `nG_a`; a DRAIN is `k nL_a`. Both first order in the holdup. **A meter's driver is a DECLARED CONSTANT** | measured with a live control: a working vapour edge gives a quotient of **2.487e+03 FLAT across probe sizes** (a real derivative); at a drained donor its worst is **0.0** |
| ⚠⚠ **the validation script's own prose stated the bug as a VIRTUE** | panel 1 read *"a meter's flux is intensive in the donor … so NOTHING IN THE FLUX LAW SLOWS IT DOWN as the funnel drains"* — an accurate description of the code, and the whole defect | `validation/dropwise.py`, corrected, with a new panel 1b |
| ⚠⚠ **the fix, and the flux law it produces** | `_smoothstep(tot/DRYOUT_MOLES)` is the GATE, `MOLE_FRACTION_DENOM` (24 decades lower) is the CLAMP. Flux becomes `k·u²(3−2u)` — **QUADRATIC in the holdup, self-limiting harder than a drain** | `test_a_metered_edge_stops_pumping_as_its_donor_dries` asserts the **closed form**, not a threshold; panel 1b matches it to every digit |
| ⚠⚠⚠ **the cap came off and the answer did not move** | **`elapsed` = 29.985000000 s at every cap from 4, 8, 10, 12, 14, 15, 20 to 60** — the same value the ten capped runs agreed on before the fix. `max_species` is back at 60 | *An answer that does not move across the fix is what says it is a fix and not a retune* — the same evidence C5 used for its cap |
| ⚠ **and the gate does not strand the charge** | `dropwise.py` panel 1 is UNCHANGED: 0.000000000000 left and 0.500000000000 delivered at every rate from 0.001 to 10 mol/s | the smoothstep's tail keeps draining. **Attenuating a flux cannot make matter, and it need not lose any** |
| ⚠⚠⚠ **C6 nearly wrote the OPPOSITE of its own finding into the engine** | *"the funnel is pumped 6.29 mmol past empty"* went into a code comment and is **FALSE**. The accepted solution has **150 points, NONE negative, bottoming at +1.500000e-04 mol**. The −6.29e-03 was measured over RHS EVALUATIONS — Newton trial iterates | **an RHS is not only evaluated on its trajectory, and a term that is defensible only there is not defensible.** *A measurement was right and the sentence drawn from it was wrong — C5's own finding, happening to C6* |
| ⚠⚠ **a docstring had gone stale in the one way that mattered** | `useful_sparsity` said the pattern is pure overhead *"for every rig in this repo's test suite"*. G1's funnel arrived later, is joined by a METER, and GROUPS | **the code was right and only the prose was wrong** — but the sparse path is the one that RAISES, so the sentence would have told a reader the crashing branch was unreachable |
| ⚠ **a latent unit mismatch, found and NOT fired** | `factor_bound` caps `\|h_j\| <= max_i \|y_i\|`, and on a rig `max\|y\|` is a **TEMPERATURE in kelvin** (356.0482) spent on a **MOLE COUNT** — a permitted probe of 356 mol into a species holding 1e-39 | **0 clamps in 20 Jacobians here**, so nothing changed. Recorded as fragility **00b** |
| the tests | **2 new**, both verified to FAIL on pre-C6 code: full rate at a 1e-8 mol donor (0.01 against 2.98e-06), quotients **[5.9e+18, 5.9e+14, 5.9e+10, 5.9e+07]** — *exactly 1/h* — and the uncapped funnel raising | `python -m pytest tests/test_rig.py tests/test_dropping_funnel.py -q` (~1 min 43 s) |
| ⚠ **what remains** | `splu`'s raise is still uncaught — C6 removed the CAUSE, not the CONSEQUENCE — and **no other `n_i / sum(n)` in the engine was swept** | fragility 00, narrowed and no longer resting on a fixed scenario. *That sweep is the obvious next numerics job and it is cheap* |
