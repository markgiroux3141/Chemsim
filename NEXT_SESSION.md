...
| ⚠⚠ **NO PROTONATION — a LIMIT to remove, not an invariant to keep** | aniline is priced as a FREE BASE at **2.8e8 x benzene**, where the real anilinium ion in mixed acid is meta-directing and SLOWER than benzene; 4-aminophenol's Σσ+ of **-2.220** is a **-82.4 kJ/mol** shift and drives the barrier through zero | `test_a_barrier_may_not_go_negative_and_the_floor_is_reachable`; `hammett.clamp_barrier`, and `build_network` emits a NOTICE naming the missing physics. **This is the next item on the aromatic branch** |
