...
The C-series is PAUSED.** Jump to `# THE P-SERIES` near the end of this file;
...
## P1 -- The notices have to have somewhere to go **(do this first)**
...
is not a place a player looks.

* Route notices into the `Snapshot` the worker publishes, so the reports panel
  shows them. The panel already exists and already shows
  `conservation_report`, `lle_report` and the rest.
* ⚠⚠⚠ **AND CLOSE THE ONE COVERAGE LIMIT THAT DOES NOT REPORT ITSELF.**
  `builder.py` breaks out of its expansion loop on the generation limit with a
  **non-empty frontier and says nothing**, while `max_species`, oversize
  molecules and mixed standard states all report. The P-series runs
  `generations=1` on every step, so this is the difference between a declared
  bound and a silent lie about the contents of a flask. See `GAME_DESIGN.md`
  §8.2, where it is the one thing that makes one-generation play admissible
  under §3's rule that no approximation may touch MATTER.

Half a session. Everything after it is easier to debug.

## P2 -- `Stock` and the shelf: the two verbs that close the loop
...
temperature, never `(name, purity)`. `SAVE_VERSION` already serialises it, so
this is mostly plumbing. Purity is DERIVED for display; impurities are carried
individually and forever, which is the whole loop: a contaminant introduced in
step 1 can ruin step 6 and the player can trace it back.
